...
// CQLCache implements the Querier interface - It will ignore the aggregation argument
func (self *CQLCache) Query(entries chan Entry, series uuid.UUID, aggregation string, start time.Time, end time.Time, success chan error) {
    seriesUUID, err := gocql.UUIDFromBytes(series)
    if err != nil {
        success <- err
        return
